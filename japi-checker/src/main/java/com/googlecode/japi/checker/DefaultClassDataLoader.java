 ...
 */
class DefaultClassDataLoader
    implements ClassDataLoader
{
    private Map<URI, AbstractClassReader> readers = new Hashtable<URI, AbstractClassReader>();


    /**
     * Read a set of classes via this ClassDataLoader. The idea is similar to the regular Java ClassLoader but
     * for ClassData type of objects.
     *
     ...
     */
    protected void read(File filename)
        throws IOException
    {
     ...
     */
    @Override
    public Map<String, ClassData> getClasses(URI uri)
    {
        Map<String, ClassData> result = new HashMap<String, ClassData>();
        AbstractClassReader reader = readers.get(uri);
        for (ClassData cd : reader.getClasses())
        {
                                      ...
                                      List<AntPatternMatcher> excludes)
    {
        Map<String, ClassData> result = new HashMap<String, ClassData>();
        AbstractClassReader reader = readers.get(uri);
        for (ClassData cd : reader.getClasses(includes, excludes))
        {
            result.put(cd.getName(), cd);
        }
